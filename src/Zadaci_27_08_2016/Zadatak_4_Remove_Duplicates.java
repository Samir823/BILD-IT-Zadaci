...
public class Zadatak_4_Remove_Duplicates {

	static Scanner input = new Scanner(System.in);

	public static void main(String[] args) {
		/*
		 * *11.13 (Remove duplicates) Write a method that removes the duplicate
		 ...
		 * displays the distinct integers separated by exactly one space. Here
		 * is a sample run:
		 */

		System.out.println("Unesite 10 cijelih brojeva: "); // unos podataka u
															// array listu
	...
	public static void removeDuplicate(ArrayList<Integer> list) {
		...
		for (int i = 0; i < list.size() - 1; i++) { // poredi element sa
													// sledecim
			for (int j = i + 1; j < list.size(); j++) {
				if (list.get(i).equals(list.get(j))) { // ako su jednaki
														// izbacuje dupli
														// element
					list.remove(j); // i dekrementira j jer su indeksi u listi
									// dinamicni (ili kako se vec to zove)
					j--;
				}
			}
		}
